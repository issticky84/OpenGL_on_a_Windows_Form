...
{
	Mat Ev = Mat::zeros(num_of_five_minutes,cluster_centers.cols,CV_32F);
	for(int i=0;i<num_of_five_minutes;i++)
	{
		float base = 0;
		for(int j=0;j<k;j++)
		{
	...
	Mat histo_coeff = Mat::zeros(num_of_five_minutes,num_of_five_minutes,CV_64F);
	//for(int i=0;i<time_step_amount;i++)
	...
	//		}
	for(int i=0;i<num_of_five_minutes;i++)
		for(int j=0;j<num_of_five_minutes;j++)
...
void Preprocessing_Data::rearrange_mat_by_sort_color_index(Mat lab_color_sort_index, Mat& cluster_center, Mat& cluster_tag, Mat& rgb_mat2)
{
	Mat cluster_center_old = cluster_center.clone();
	Mat cluster_tag_old = cluster_tag.clone();
	Mat rgb_mat2_old = rgb_mat2.clone();
	int k = cluster_center.rows;
	int raw_data_size = cluster_tag.rows;
...
void Preprocessing_Data::sort_histogram_by_Ev_by_TSP_coarse_to_fine(Mat cluster_center, Mat& histo_sort_index)
{
	...
	//TSP_boost(lab_cluster_center2D, color_sort_index);
	//TSP_path(group_cluster_center_2D, group_sort_index);
	...
	////////////////////////////////////////////////////////////////////////////////////
	//Mat Ev_PCA2D_sub = Mat::zeros(groups[0].index2.size()+1,2,CV_32F);
	...
	{
		int index = groups[0].index2[i];
		//Ev_sub.at<float>(i,0) = Ev.at<float>(index,0);
		//Ev_sub.at<float>(i,1) = Ev.at<float>(index,1);
	...
	{
		int index = groups[1].index2[j];
		//Ev_sub.at<float>(groups[0].index2.size(),0) = Ev.at<float>(index,0);
		//Ev_sub.at<float>(groups[0].index2.size(),1) = Ev.at<float>(index,1);
	...
	cout << endl;
	////////////////////////////////////////////////////////////////////////////////////
	//Ev_PCA2D_sub = Mat::zeros(groups[0].sort_index.size()+1,2,CV_32F);
	...
	{
		int index = groups[2].index2[j];
		//Ev_sub.at<float>(groups[0].sort_index.size(),0) = Ev.at<float>(index,0);
		//Ev_sub.at<float>(groups[0].sort_index.size(),1) = Ev.at<float>(index,1);		
	...
	cout << "start " << start << endl;
	///////////////////////////////////////////////////////////////////////
	//Mat Ev_PCA2D_sub = Mat::zeros(groups[2].index2.size()+1,2,CV_32F);
	Ev_sub = Mat::zeros(groups[2].index2.size()+1,Ev.cols,CV_32F);
	histo_sort_index_sub = Mat::zeros(groups[2].index2.size()+1,1,CV_32S);	
	for(int i=0;i<groups[2].index2.size();i++)
	{
		int index = groups[2].index2[i];
		//Ev_PCA2D_sub.at<float>(i,0) = Ev_PCA2D.at<float>(index,0);
		//Ev_PCA2D_sub.at<float>(i,1) = Ev_PCA2D.at<float>(index,1);		
		Ev.row(index).copyTo( Ev_sub.row(i) );
	}
	min_len = 10000;
	min_target = 0;
	...
	{
		int index = groups[3].index2[j];
		//Ev_PCA2D_sub.at<float>(groups[2].index2.size(),0) = Ev_PCA2D.at<float>(index,0);
		//Ev_PCA2D_sub.at<float>(groups[2].index2.size(),1) = Ev_PCA2D.at<float>(index,1);		
	...
	//Ev_sub.at<float>(0,0) = Ev.at<float>(start,0);
	//Ev_sub.at<float>(0,1) = Ev.at<float>(start,1);
